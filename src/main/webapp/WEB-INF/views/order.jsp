...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/resources/css/order.css?after"/>
	<script
					...
					<table class="table_text_align_center memberInfo_table">
						<tbody>
							<tr>
								...
								<th style="width: *">${memberInfo.member_name } | ${memberInfo.member_email}</th>
							</tr>
						</tbody>
					</table>
				</div>
					...
					<table class="goods_table">
						...
						<tbody>
							<c:forEach items="${orderList }" var="ol">
								<tr>
									<td>
										<div class="image_wrap" data-bookid="${ol.imageList[0].book_id}" data-path="${ol.imageList[0].uploadPath}" data-uuid="${ol.imageList[0].uuid}" data-filename="${ol.imageList[0].fileName }">
											<img>
										</div>
									</td>
									<td>${ol.book_name }</td>
					...
					<div class="total_info_price_div">
						<ul>
							...
							<li class="price_total_li">
								<strong class="price_span_label total_price_label">최종결재금액</strong>
...
function showAddress(className){
	...
	/* selectAddres T/F */
		...
		$(".addressInfo_input_div" + fileName).find(".selectAddress").val("T");
}
...
/* 포인트 모두사용, 취소 버튼 */ 
	/* 모두 사용 */
	$(".order_point_input_btn_N").on("click", function(){
		const maxPoint = parseInt('${memberInfo.point}');	
		//값 변경
	...
	/* 취소 */
	$(".order_point_input_btn_Y").on("click", function(){
		//값 변경
...
function setTotalInfo(){
	...
	$(".goods_table_price_td").each(function(index, element){
		totalPrice += parseInt($(element).find(".individual_total_price_input").val());		// 총 가격
		totalKind += 1;																		// 총 종류
		totalCount += parseInt($(element).find(".individual_book_count_input").val());		// 총 갯수
		totalPoint += parseInt($(element).find(".individual_totalPoint_input").val());		// 총 마일리지
	});	
